...
public class IslandHistoryTests
{
    ...
    {
        var text = Text(Scenario("43-history-no-covers"));
    ...
    {
        var playing = Text(Scenario("37-history"));
        ...
        var ended = Text(Scenario("39-history-clear-armed")); // SetFailed
    ...
    public void EmptyState() =>
        Assert.Contains(HistoryRules.EmptyText, Text(Scenario("38-history-empty")));
    ...
    public void ClearIsTwoStepAndTheGameIsNotTouched()
    {
        IslandSta.Run(() =>
        {
            var (root, view, session) = IslandSnapshots.Compose(Scenario("43-history-no-covers"));
    ...
    public void ArmedSnapshotShowsTheQuestion() =>
        Assert.Contains(HistoryRules.ClearConfirmLabel, Text(Scenario("39-history-clear-armed")));
    ...
    [Theory]
    [InlineData("40-correct-cover")]
    [InlineData("41-revealed-cover")]
    public void AnswerScreensShowTheCover(string name) => Assert.Single(Covers(Scenario(name)));

    [Fact]
    public void OfflineAnswerHasNoCover() => Assert.Empty(Covers(Scenario("42-correct-cover-offline")));
    ...
    public void HistoryRowsGetThumbnailsOrTheNote()
    {
        IslandSta.Run(() =>
        {
            var root = IslandSnapshots.Compose(Scenario("37-history")).Root;

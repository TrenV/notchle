...
public class HistoryEngineTests
{
    ...
    // The set-end path: an error on the LAST track, then Next, ends the set and must still record
    // that track (a merge once dropped it and no other test noticed).
    [Fact]
    public void NextFromAnErrorOnTheLastTrackRecordsItAtTheSetEnd()
    {
        var e = Engine(tracks: 1);
        var track = e.State.CurrentTrack!;
        e.Send(new PlaybackFailed("x"));
        var effects = e.Send(new Next());
        Assert.IsType<GamePhase.SetFailed>(e.State.Phase);
        Assert.Equal(new Effect.RecordOutcome(track, new TrackOutcome.Missed(), 0, 0), Assert.Single(Records(effects)));
    }

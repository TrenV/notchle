...
// (Enter / Tab / Ctrl+R / Ctrl+Shift+R / Ctrl+Shift+S / Ctrl+N / Ctrl+1 / Ctrl+2 / Esc); the Windows-only rules (no auto-expand, hover
...
public enum IslandKey { Enter, Escape, Tab, BackTab, CtrlR, CtrlShiftR, CtrlShiftS, CtrlN, Ctrl1, Ctrl2 }
...
public static class IslandRules
{
    ...
    /// then only disarms it. <paramref name="historyShown"/>: the History tab is up; the game
    /// keeps running behind it, but only Esc (back to Play) and the tab / quit keys act there, so
    /// no key press can guess, skip or give up unseen.
    public static IslandCommand? Command(IslandKey key, GamePhase phase, IslandField? focused, bool settingsOpen = false,
        GameConfig? config = null, bool quitArmed = false, bool historyShown = false)
    {

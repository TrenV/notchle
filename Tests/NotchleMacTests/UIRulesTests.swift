...
/// With the History tab open, no end-of-set key may start a set you can't see.
@Test func setEndKeysDoNothingWhileHistoryIsOpen() {
    for key in [UIKeyInput.returnKey, .commandShiftR, .commandShiftN] {
        #expect(NotchUIRules.command(for: key, phase: .setFailed(correctCount: 3), focused: nil,
                                     historyOpen: true, availableNew: 7) == nil, "\(key)")
    }
}

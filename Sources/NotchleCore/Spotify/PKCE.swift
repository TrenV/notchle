...
/// Proof Key for Code Exchange (RFC 7636), S256 method. SHA-256 is implemented here because
/// NotchleCore is Foundation-only (no CryptoKit on Windows).
public enum PKCE {
    /// 64 random bytes → 86 base64url characters (the RFC allows 43–128 unreserved characters).
    public static func makeVerifier() -> String { base64URL(randomBytes(64)) }
    ...
    public static func base64URL<Bytes: Sequence>(_ bytes: Bytes) -> String where Bytes.Element == UInt8 {
...
/// FIPS 180-4 SHA-256. Only used for the PKCE challenge (short inputs), so clarity over speed.
public enum SHA256 {
